require import Int IntDiv StdOrder Ring. 
require import List.
...
apply leq_div2r. smt(W64.to_uint_cmp). smt(gt0_pow2).
qed.
...
proof.
move=> *. rewrite /ubW64. rewrite to_uint_and_mod. smt().
...
lemma bW64D nx ny (x y: W64.t):
 ...
 proof.
  ...
  smt(). smt(ler_weexpn2l). smt(ler_weexpn2l).
qed.
...
proof.
  ...
  move =>*. rewrite (exprD_nneg (2) (nx) (ny)). smt().  smt().
  rewrite (W64.to_uintM x y).
  have _: to_uint x * to_uint y < 2^nx * 2^ny. apply ge_mul4.
  split; by smt(W64.to_uint_cmp). smt(W64.to_uint_cmp).
qed.
...
have _: 0 <= (to_uint x). smt(W64.to_uint_cmp).
case: (2^max 0 n = 2). move => *; by smt().
case: (2^max 0 n = 2^n). move => *; smt(expr_gt0). smt(expr_gt0).
qed.
...
rewrite to_uint_and_mod. smt(). smt(W64.to_uint_cmp).
qed.
...
proof.
elim: x c => //= x xs IH c H /=.
rewrite /nth_limbs64 /nth_digits /=; case: (size xs = 0) => /= E H0.
 ...
 by apply ubW641.
have E2 H2 : 0 < size xs by smt(size_ge0).
have := IH (carry_add W64.zero x c) E2 H0 H2.
case: (carryprop_limbs64 _ _) => /= H4 H5.
rewrite !val_digits_cons H5 E /=.
split. 
by move: (W64.addcP W64.zero x c); rewrite addcE /#. smt().
qed.
...
rewrite -IH -mulhiP mul1hi_limbsE; ring. smt().
qed.
