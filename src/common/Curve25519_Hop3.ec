...
import Zp_25519 Zp_25519.ZModpRing Curve25519_Spec Curve25519_Hop1 Curve25519_Hop2 Ring.IntID.

(** step 1 : decode_scalar_25519 **)
lemma ill_decode_scalar_25519 : islossless MHop2.decode_scalar by islossless.

lemma eq_h3_decode_scalar_25519 k:
    phoare [MHop2.decode_scalar : 
        k' = k
        ==> 
        res = decodeScalar25519 k] = 1%r.
proof. 
    by conseq ill_decode_scalar_25519 (eq_h2_decode_scalar k). 
qed.

(** step 2 : decode_u_coordinate **)
lemma ill_decode_u_coordinate : islossless MHop2.decode_u_coordinate by islossless.

lemma ill_decode_u_coordinate_base : islossless MHop2.decode_u_coordinate_base by islossless.

lemma eq_h3_decode_u_coordinate u:
    phoare [MHop2.decode_u_coordinate : 
        u' = u
        ==> 
        res = decodeUCoordinate u] = 1%r.
proof. 
    by conseq ill_decode_u_coordinate (eq_h2_decode_u_coordinate u). 
qed.
  
(** step 3 : ith_bit **)
lemma ill_ith_bit : islossless MHop2.ith_bit by islossless.

lemma eq_h3_ith_bit (k : W256.t) i:
    phoare [MHop2.ith_bit : 
        k' = k /\ 
        ctr = i 
        ==> 
        res = ith_bit k i] = 1%r.
proof. 
    by conseq ill_ith_bit (eq_h2_ith_bit k i). 
qed.

(** step 4 : cswap **)
lemma ill_cswap : islossless MHop2.cswap by islossless.
...
proof.    
    by conseq ill_cswap (eq_h2_cswap t b). 
qed.

(** step 5 : add_and_double **)
lemma ill_add_and_double : islossless MHop2.add_and_double by islossless.

lemma eq_h3_add_and_double (qx : zp) (nqs : (zp * zp) * (zp * zp)):
    phoare [MHop2.add_and_double : 
        init = qx /\ 
        x2 = nqs.`1.`1 /\
        z2 = nqs.`1.`2 /\
        x3 = nqs.`2.`1 /\
...
proof. 
    by conseq ill_add_and_double (eq_h2_add_and_double qx nqs). 
qed.

(** step 6 : montgomery_ladder_step **)
lemma ill_montgomery_ladder_step : islossless MHop2.montgomery_ladder_step by islossless.

lemma eq_h3_montgomery_ladder_step (k : W256.t) (init : zp) (nqs : (zp * zp) * (zp * zp) * bool) (ctr : int) :
    phoare [MHop2.montgomery_ladder_step : 
        k' = k           /\ 
        init' = init     /\
        x2 = nqs.`1.`1   /\
        z2 = nqs.`1.`2   /\
...
proof. 
    by conseq ill_montgomery_ladder_step (eq_h2_montgomery_ladder_step k init nqs ctr). 
qed.
...
lemma eq_h3_montgomery_ladder (init : zp) (k : W256.t) :
  phoare [MHop2.montgomery_ladder : 
        init' = init /\
        k.[0] = false /\
        k' = k
        ==> 
...
lemma eq_h3_it_sqr (e : int) (z : zp) : 
    phoare[MHop2.it_sqr :
        ...
        res = it_sqr1 e z] = 1%r.
proof. 
    by conseq ill_it_sqr (eq_h2_it_sqr e z). 
qed.
...
lemma eq_h3_encode_point (q : zp * zp) : 
    phoare[MHop2.encode_point : 
        x2 =  q.`1 /\ z2 = q.`2 
        ==> 
        res = encodePoint1 q] = 1%r.
proof. 
