require import List Int IntDiv StdOrder Ring. 
...
op val_digits (base: int) (x: int list) = foldr (fun w r => w + base * r) 0 x.

lemma val_digitsE base x: val_digits base x = foldr (fun w r => w + base * r) 0 x by done.
...
proof.
    elim: x => //= x xs IH /=.
    rewrite !val_digits_cons IH exprD_nneg //.
    by rewrite expr1; ring.   
qed.
...
proof. 
    rewrite /ubW64. split => *.
        have <-: W64.of_int (to_uint x) = W64.of_int 0.
            congr. trivial. by smt(W64.to_uint_cmp).
        rewrite to_uintK. trivial.
    case: (x=W64.of_int 0); trivial.
qed.
...
lemma ubW64D nx ny (x y: W64.t):
    ...
    ubW64 (nx + ny) (x + y).
proof. 
    by rewrite /ubW64 to_uintD; smt (le_modz W64.to_uint_cmp). 
qed.
...
lemma ubW64M nx ny (x y: W64.t):
    ...
    ubW64 (nx * ny) (x * y).
proof. 
    rewrite /ubW64 to_uintM; smt(le_modz W64.to_uint_cmp). 
qed.
...
lemma ubW64shr k n (x: W64.t):
    ...
    ubW64 (n %/ 2^k) (x `>>>` k).
proof.
    move=> *; rewrite /ubW64 to_uint_shr //.
    apply leq_div2r. 
        smt(W64.to_uint_cmp). 
    smt(gt0_pow2).
qed.
...
proof.
    move=> *. rewrite /ubW64. rewrite to_uint_and_mod.
         smt().
...
proof. 
    by rewrite bW64ub //; apply ubW64T. 
qed.

lemma bW64_0: bW64 0 W64.zero.
proof. 
    by rewrite bW64E W64.to_uint0. 
qed.

lemma bW64_1: bW64 1 W64.one.
proof. 
    by rewrite bW64E W64.to_uint1. 
qed.
...
proof.
    ...
    apply (ubW64W (2 ^ n1 - 1)) => //.
    apply ler_sub => //. case: (0 <= n2). move => *.
    apply (ler_weexpn2l); trivial. smt(ler_weexpn2l). 
qed.
...
proof.
    ...
    apply (lez_trans (2^(max nx ny + 1)-1)).
        rewrite exprS 1:/#.
        case: (nx <= ny) => ?.
            apply (ler_trans (2 ^ ny - 1 + (2 ^ ny - 1))); first smt(ler_weexpn2l).
            smt(). 
        smt(ler_weexpn2l). 
    smt(ler_weexpn2l).
qed.
...
axiom ge_mul4 (x y u v: int): 0 <= x && 0 <= y && x < u && y < v => x * y < u * v.
...
lemma bW64M nx ny (x y: W64.t):
    ...
    bW64 (nx + ny) (x * y).
proof.
    rewrite /bW64. move => *. split; first smt().
    move =>*. rewrite (exprD_nneg (2) (nx) (ny)).
            smt().  
        smt().
    rewrite (W64.to_uintM x y).
    have _: to_uint x * to_uint y < 2^nx * 2^ny. 
        apply ge_mul4.
        split; by smt(W64.to_uint_cmp). 
    smt(W64.to_uint_cmp).
qed.
...
proof. 
    by move => ?; apply (bW64W 64) => //; apply bW64T. 
qed.
...
proof. 
    by move=> *; rewrite -W64.shr_shrw //; apply bW64_shr. 
qed.
...
proof. 
    by move=> *; rewrite -W64.shl_shlw //; apply bW64_shl. 
qed.
...
proof.
    ...
    rewrite -exprD_nneg //; apply ler_weexpn2l; smt().
qed.
...
proof.
    have ->: val_limbs base (x ++ y) = val_digits base (digits64 x ++ digits64 y) by rewrite -map_cat.
...
axiom mul1_limbsE' (x: W64.t) (y: W64.t list): mul1_limbs x y = map( ( * ) x) y.
...
proof. 
    by elim: x => //= x xs; split; first apply bW64_0. 
qed.
...
proof. 
    by rewrite /= val_digits_cons. 
qed.

lemma val_limbs8_cons x xs:
    val_limbs8 (x::xs) = to_uint x + 2^8 * val_limbs8 xs.
proof. 
    by rewrite /= val_digits_cons. 
qed.

op nth_limbs64 (x: W64.t list) (n: int) = nth W64.zero x n.
op nth_limbs8  (x: W8.t list)  (n: int) = nth W8.zero x n.
...
proof.
    ...
    have ->: 2 ^ (64 * (1 + size xs)) * b2i x1 = 2^64*2^(64*size xs)*b2i x1.
        by rewrite mulzDr exprD_nneg //; smt(size_ge0).
    smt().
qed.
...
proof.
    ...
    rewrite /nth_limbs64 /nth_digits /=; case: (size xs = 0) => /= E H1.
        rewrite size_eq0 in E; rewrite E carryprop_limbs64_nil /val_digits /=.
        move=> H2; split; first by rewrite to_uintD_small of_uintK; smt(modz_small).
        apply (ubW64D _ _ _ (W64.of_int (b2i c)) H2).
        by apply ubW641.
    have E2 H3 : 0 < size xs by smt(size_ge0).
    have := IH (carry_add W64.zero x c) E2 H1 H3.
    case: (carryprop_limbs64 _ _) => /= H4[H5 H6].
    rewrite !val_digits_cons H5 E /=.
    split; last by [].
...
lemma add_limbs64_cons x xs y ys c:
    add_limbs64 (x::xs) (y::ys) c = 
        let (c',z') = add_limbs64 xs ys (addc_carry x y c)
        in (c', (x+y+W64.of_int (b2i c)) :: z').
proof. 
    by rewrite /= !addcE /=; case: (add_limbs64 xs ys (carry_add x y c)). 
qed.
...
proof. 
    by rewrite /add_limbs64nc; apply (size_add_limbs64 x y false).     
qed.
...
proof. 
    by move=> *; have [? _] := add_limbs64nc_aux nx ny x y false _ _ _ _. 
qed.
...
proof. 
    by move=> *; have ? := add_limbs64nc_aux nx ny x y false _ _ _ _. 
qed.
